...
public class TFieldSignatureImpl extends TSignatureImpl implements TFieldSignature {
	...
	public EList<TFieldDefinition> getDefinitions() {
		if (definitions == null) {
			definitions = new EObjectContainmentWithInverseEList<TFieldDefinition>(TFieldDefinition.class, this,
					BasicPackage.TFIELD_SIGNATURE__DEFINITIONS, BasicPackage.TFIELD_DEFINITION__SIGNATURE);
		}
		return definitions;
	}
	...
	public TField getField() {
		if (eContainerFeatureID() != BasicPackage.TFIELD_SIGNATURE__FIELD)
			return null;
	...
	public void setField(TField newField) {
		if (newField != eInternalContainer()
				|| (eContainerFeatureID() != BasicPackage.TFIELD_SIGNATURE__FIELD && newField != null)) {
			...
			if (newField != null)
				msgs = ((InternalEObject) newField).eInverseAdd(this, BasicPackage.TFIELD__SIGNATURES, TField.class,
						msgs);
			msgs = basicSetField(newField, msgs);
			if (msgs != null)
				msgs.dispatch();
		} else if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, BasicPackage.TFIELD_SIGNATURE__FIELD, newField,
					newField));
	}
		...
		if (type != null && type.eIsProxy()) {
			...
			if (type != oldType) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, BasicPackage.TFIELD_SIGNATURE__TYPE,
							oldType, type));
			}
		}
		return type;
